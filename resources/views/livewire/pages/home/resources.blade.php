<div class="max-w-[85rem] px-4 py-10 sm:px-6 lg:px-8 lg:py-14 mx-auto">
    <div class="max-w-3xl mx-auto text-center">
        ...
        <p class="mt-4 text-gray-600 dark:text-neutral-400">
            Acesse uma variedade de documentos, tutoriais e guias para apoiar seu aprendizado e desenvolvimento.
        </p>
    </div>
        ...
        @foreach ($resources as $resource)
            <div class="flex flex-col border rounded-lg shadow-lg overflow-hidden dark:border-neutral-700">
                <div class="flex-1 p-6 bg-white dark:bg-neutral-800">
                    <h2 class="text-xl font-semibold text-gray-800 dark:text-neutral-200">{{ $resource['title'] }}</h2>
                    <p class="mt-3 text-sm text-gray-600 dark:text-neutral-400">{{ $resource['description'] }}</p>
                </div>
                <div class="p-4 bg-gray-50 dark:bg-neutral-900">
                    <div class="flex items-center justify-between text-sm text-gray-500 dark:text-neutral-400">
                        <span>Data: {{ date('d M Y', strtotime($resource['date'])) }}</span>
                    </div>
                </div>
                <div class="p-4">
                    <a href="{{ $resource['link'] }}" class="text-blue-600 hover:underline dark:text-blue-400">Acessar Recurso</a>
                </div>
            </div>
        @endforeach
    </div>
